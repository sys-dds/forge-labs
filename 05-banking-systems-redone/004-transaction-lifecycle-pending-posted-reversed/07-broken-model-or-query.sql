...
WITH latest_state AS (SELECT external_transaction_id, MAX(event_state) AS event_state, MAX(amount_cents) AS amount_cents FROM transaction_events GROUP BY external_transaction_id), posted_balance AS (SELECT account_id, SUM(amount_cents) AS balance_cents FROM transaction_events WHERE event_state IN ('posted','pending') GROUP BY account_id), pending AS (SELECT account_id, SUM(amount_cents) AS pending_cents FROM transaction_events WHERE event_state='pending' GROUP BY account_id)
