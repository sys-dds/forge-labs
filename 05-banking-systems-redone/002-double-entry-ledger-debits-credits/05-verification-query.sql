...
WITH tx_totals AS (
  SELECT lt.ledger_transaction_id, SUM(le.amount_cents) FILTER (WHERE le.side='debit') AS debit_total, SUM(le.amount_cents) FILTER (WHERE le.side='credit') AS credit_total, COUNT(*) FILTER (WHERE le.side='debit') AS debit_rows, COUNT(*) FILTER (WHERE le.side='credit') AS credit_rows, BOOL_AND(a.currency=lt.currency) AS same_currency
  FROM ledger_transactions lt JOIN ledger_entries le USING (ledger_transaction_id) JOIN accounts a USING (account_id) GROUP BY lt.ledger_transaction_id
), balances AS (
