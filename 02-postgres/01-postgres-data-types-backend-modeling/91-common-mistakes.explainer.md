...
The tempting shortcut is to check `money stored as float, timestamps stored as text, flags used for workflow state` in one application handler. That misses imports, retries, admin scripts, and future services. The chapter proof forces the database behavior to be visible.
