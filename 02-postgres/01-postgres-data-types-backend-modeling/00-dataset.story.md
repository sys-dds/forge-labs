...
Ada buyer, Ben seller, Cy seller, Maya operator, one listing, one request, one trust signal, one payment boundary event.
...
money stored as float, timestamps stored as text, flags used for workflow state.

## Proof Assertions

exact numeric values, timestamptz ordering, uuid shape, rejected empty handles.
