...
This chapter focuses on types, timestamptz, numeric, uuid, boolean, jsonb. The feature matters because the database can reject or shape data before a backend handler accidentally spreads a bad assumption across many code paths.

## Backend Reason

Ada buyer, Ben seller, Cy seller, Maya operator, one listing, one request, one trust signal, one payment boundary event. If this rule is only implied in application code, a retry, import job, admin edit, or second service can bypass it. PostgreSQL gives the rule a stable home next to the data.
