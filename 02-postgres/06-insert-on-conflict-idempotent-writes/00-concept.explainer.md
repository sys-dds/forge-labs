...
This chapter focuses on ON CONFLICT DO NOTHING, DO UPDATE, scoped idempotency keys. The feature matters because the database can reject or shape data before a backend handler accidentally spreads a bad assumption across many code paths.

## Backend Reason

Ada retries create request, Ben uses the same key in a different actor scope, status update is retried. If this rule is only implied in application code, a retry, import job, admin edit, or second service can bypass it. PostgreSQL gives the rule a stable home next to the data.
