...
This chapter focuses on enum status, text arrays, tstzrange, generated labels. The feature matters because the database can reject or shape data before a backend handler accidentally spreads a bad assumption across many code paths.

## Backend Reason

Ben has active cleaning listing, Cy has paused repair listing, Ada searches tags and time windows. If this rule is only implied in application code, a retry, import job, admin edit, or second service can bypass it. PostgreSQL gives the rule a stable home next to the data.
