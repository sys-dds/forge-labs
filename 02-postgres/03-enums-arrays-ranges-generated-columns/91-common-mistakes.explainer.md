...
The tempting shortcut is to check `status text with typos, arrays replacing every relationship, generated columns edited by hand` in one application handler. That misses imports, retries, admin scripts, and future services. The chapter proof forces the database behavior to be visible.
