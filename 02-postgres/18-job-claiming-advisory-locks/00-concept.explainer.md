...
This chapter focuses on job claiming, due filters, worker ownership, status transition. The feature matters because the database can reject or shape data before a backend handler accidentally spreads a bad assumption across many code paths.

## Backend Reason

due email job, not-yet-due cleanup, running export, failed retryable sync. If this rule is only implied in application code, a retry, import job, admin edit, or second service can bypass it. PostgreSQL gives the rule a stable home next to the data.
