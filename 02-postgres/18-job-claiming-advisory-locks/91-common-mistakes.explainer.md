...
The tempting shortcut is to check `no due_at filter, advisory lock without row update, no worker id` in one application handler. That misses imports, retries, admin scripts, and future services. The chapter proof forces the database behavior to be visible.
