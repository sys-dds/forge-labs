...
The tempting shortcut is to check `treating NOTIFY as durable queue, oversized payloads, offline consumers` in one application handler. That misses imports, retries, admin scripts, and future services. The chapter proof forces the database behavior to be visible.
