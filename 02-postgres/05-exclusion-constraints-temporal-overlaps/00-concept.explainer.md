...
This chapter focuses on range types, btree_gist, exclusion constraints. The feature matters because the database can reject or shape data before a backend handler accidentally spreads a bad assumption across many code paths.

## Backend Reason

Ben accepts 09:00 booking, adjacent 10:00 booking, Cy accepts overlapping window, Ada tries overlapping Ben booking. If this rule is only implied in application code, a retry, import job, admin edit, or second service can bypass it. PostgreSQL gives the rule a stable home next to the data.
