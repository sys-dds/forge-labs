...
The tempting shortcut is to check `app-only overlap checks, inclusive bounds, provider scope missing` in one application handler. That misses imports, retries, admin scripts, and future services. The chapter proof forces the database behavior to be visible.
