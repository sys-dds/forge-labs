...
This chapter focuses on RLS enablement, current_setting tenant context, app role policies. The feature matters because the database can reject or shape data before a backend handler accidentally spreads a bad assumption across many code paths.

## Backend Reason

Workspace alpha has Ada listing, beta has Ben listing, app_user switches tenant context. If this rule is only implied in application code, a retry, import job, admin edit, or second service can bypass it. PostgreSQL gives the rule a stable home next to the data.
