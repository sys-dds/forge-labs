...
The tempting shortcut is to check `status update without audit, missing invalid-transition exception` in one application handler. That misses imports, retries, admin scripts, and future services. The chapter proof forces the database behavior to be visible.
