...
This chapter focuses on PL/pgSQL, exceptions, audit writes, controlled mutation. The feature matters because the database can reject or shape data before a backend handler accidentally spreads a bad assumption across many code paths.

## Backend Reason

Cy dispute starts open, Maya resolves it, invalid repeat resolution is attempted. If this rule is only implied in application code, a retry, import job, admin edit, or second service can bypass it. PostgreSQL gives the rule a stable home next to the data.
