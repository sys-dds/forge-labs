...
This chapter focuses on BEFORE UPDATE updated_at, AFTER trigger audit rows, OLD/NEW. The feature matters because the database can reject or shape data before a backend handler accidentally spreads a bad assumption across many code paths.

## Backend Reason

Ben listing changes from active to paused and audit captures old/new values. If this rule is only implied in application code, a retry, import job, admin edit, or second service can bypass it. PostgreSQL gives the rule a stable home next to the data.
