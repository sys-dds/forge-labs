...
The tempting shortcut is to check `app-only audit responsibility, missing OLD/NEW, updated_at stale` in one application handler. That misses imports, retries, admin scripts, and future services. The chapter proof forces the database behavior to be visible.
