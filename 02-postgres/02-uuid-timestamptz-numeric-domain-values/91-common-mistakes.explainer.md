...
The tempting shortcut is to check `external ids built from names, negative amounts, empty request references` in one application handler. That misses imports, retries, admin scripts, and future services. The chapter proof forces the database behavior to be visible.
