...
This chapter focuses on jsonb payloads, containment, optional keys, typed core columns. The feature matters because the database can reject or shape data before a backend handler accidentally spreads a bad assumption across many code paths.

## Backend Reason

listing_created, review_flagged, payment_boundary_event, moderation_event, optional metadata missing. If this rule is only implied in application code, a retry, import job, admin edit, or second service can bypass it. PostgreSQL gives the rule a stable home next to the data.
