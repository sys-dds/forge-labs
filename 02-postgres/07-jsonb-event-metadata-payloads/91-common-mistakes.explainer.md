...
The tempting shortcut is to check `core facts buried only in JSONB, assuming identical payload shape` in one application handler. That misses imports, retries, admin scripts, and future services. The chapter proof forces the database behavior to be visible.
