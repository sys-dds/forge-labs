...
This chapter focuses on materialized views, refresh, stale summaries. The feature matters because the database can reject or shape data before a backend handler accidentally spreads a bad assumption across many code paths.

## Backend Reason

Ben starts with one verified review, Maya adds another after materialization, normal view sees it immediately. If this rule is only implied in application code, a retry, import job, admin edit, or second service can bypass it. PostgreSQL gives the rule a stable home next to the data.
