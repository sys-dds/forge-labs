...
This chapter focuses on jsonb extraction, shape checks, generated projection columns. The feature matters because the database can reject or shape data before a backend handler accidentally spreads a bad assumption across many code paths.

## Backend Reason

risk event with numeric score, moderation event reason, provider event external id, malformed missing external id. If this rule is only implied in application code, a retry, import job, admin edit, or second service can bypass it. PostgreSQL gives the rule a stable home next to the data.
