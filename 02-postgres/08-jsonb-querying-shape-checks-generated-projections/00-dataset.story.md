...
risk event with numeric score, moderation event reason, provider event external id, malformed missing external id.
...
numeric JSON read as text, no required key check, generated column manually set.

## Proof Assertions

projection equals JSON value, malformed payload rejected, exact risk events.
