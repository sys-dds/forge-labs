...
The tempting shortcut is to check `numeric JSON read as text, no required key check, generated column manually set` in one application handler. That misses imports, retries, admin scripts, and future services. The chapter proof forces the database behavior to be visible.
