...
This chapter focuses on pg_try_advisory_xact_lock, lock keys, local coordination. The feature matters because the database can reject or shape data before a backend handler accidentally spreads a bad assumption across many code paths.

## Backend Reason

nightly reconciliation and payout sweep jobs use different lock keys. If this rule is only implied in application code, a retry, import job, admin edit, or second service can bypass it. PostgreSQL gives the rule a stable home next to the data.
