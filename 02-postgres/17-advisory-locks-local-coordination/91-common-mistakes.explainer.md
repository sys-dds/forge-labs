...
The tempting shortcut is to check `same key for unrelated jobs, assuming lock protects rows automatically` in one application handler. That misses imports, retries, admin scripts, and future services. The chapter proof forces the database behavior to be visible.
