...
This chapter focuses on USING vs WITH CHECK, read/write policies, tenant insert protection. The feature matters because the database can reject or shape data before a backend handler accidentally spreads a bad assumption across many code paths.

## Backend Reason

Alpha and beta workspaces, app_user tries cross-tenant insert and update. If this rule is only implied in application code, a retry, import job, admin edit, or second service can bypass it. PostgreSQL gives the rule a stable home next to the data.
