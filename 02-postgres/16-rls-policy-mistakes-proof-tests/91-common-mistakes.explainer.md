...
The tempting shortcut is to check `USING without WITH CHECK, write policy missing, happy-path-only RLS tests` in one application handler. That misses imports, retries, admin scripts, and future services. The chapter proof forces the database behavior to be visible.
