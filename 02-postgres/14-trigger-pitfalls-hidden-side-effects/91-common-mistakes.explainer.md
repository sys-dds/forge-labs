...
The tempting shortcut is to check `recursive trigger, no WHEN guard, surprise bulk audit volume` in one application handler. That misses imports, retries, admin scripts, and future services. The chapter proof forces the database behavior to be visible.
