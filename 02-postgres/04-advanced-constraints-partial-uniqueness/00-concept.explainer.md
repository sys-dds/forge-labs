...
This chapter focuses on composite uniqueness, partial unique indexes, CHECK constraints. The feature matters because the database can reject or shape data before a backend handler accidentally spreads a bad assumption across many code paths.

## Backend Reason

Ben has active bicycle-repair listing, Ben also has removed history with same slug, Cy uses same slug safely. If this rule is only implied in application code, a retry, import job, admin edit, or second service can bypass it. PostgreSQL gives the rule a stable home next to the data.
