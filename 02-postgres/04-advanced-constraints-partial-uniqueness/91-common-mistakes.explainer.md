...
The tempting shortcut is to check `global slug uniqueness, app-only active checks, nullable uniqueness assumptions` in one application handler. That misses imports, retries, admin scripts, and future services. The chapter proof forces the database behavior to be visible.
