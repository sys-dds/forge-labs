...
This chapter focuses on views, API-shaped projections, hidden private columns, aggregation. The feature matters because the database can reject or shape data before a backend handler accidentally spreads a bad assumption across many code paths.

## Backend Reason

Ben and Cy sellers with listings, reviews, disputes, private emails. If this rule is only implied in application code, a retry, import job, admin edit, or second service can bypass it. PostgreSQL gives the rule a stable home next to the data.
