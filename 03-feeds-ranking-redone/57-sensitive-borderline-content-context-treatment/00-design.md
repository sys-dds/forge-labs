...
Sensitive borderline content context treatment teaches how safety evidence changes candidate treatment before, during, and after ranking.
...
- Item: Ada and Ben contextual feeds
- Why it exists: expected safe output or decision surface for sensitive borderline content context treatment.
...
`same feed for every viewer`

## Correct result from the solution

`Ada and Ben contextual feeds`
...
Explain sensitive borderline content context treatment with the row that proves the risk and the treatment that ranking receives.
