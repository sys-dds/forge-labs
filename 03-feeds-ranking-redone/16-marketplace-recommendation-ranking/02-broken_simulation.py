...
def run(dataset_path):
    data=load_dataset(dataset_path); ranked=sorted(data["content"], key=lambda r:r.get("price",999999)); ids=[r["id"] for r in ranked[:5]]
    return {"inventory":inventory(data),"eligible_candidates":ranked,"rejected_items":[],"marketplace_recommendations":ids,"final_feed":ids,"debug_trace":[]}
