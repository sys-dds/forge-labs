...
Social graph data model teaches edge direction and edge type as candidate generation before final ranking.
...
- Item: post_101 and maya
- Why it exists: expected graph candidate set for edge direction and edge type.
...
Explain edge direction and edge type with the exact rows in this clinic and one failure mode that would hurt the feed.
