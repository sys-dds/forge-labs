...
|---|---|---|---|---|
| post_101 | Primary kept candidate for authority propagation from trusted sources. | Proof asserts it appears in graph candidates. | Trace names its graph path or score. | Drill removes its source path. |
...
| score/path row | Makes authority propagation from trusted sources visible. | Proof checks semantic row fields. | Trace includes path or numeric value. | Drill erases row details. |
