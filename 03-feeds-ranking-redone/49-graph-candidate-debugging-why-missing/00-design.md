...
Graph candidate debugging why missing teaches stage-specific graph missing explanations as candidate generation before final ranking.
...
- Item: post_101, maya, backend_club, post_701
- Why it exists: expected graph candidate set for stage-specific graph missing explanations.
...
`[post_101, maya, backend_club]`

## Correct result from the solution

`post_101, maya, backend_club, post_701`
...
Explain stage-specific graph missing explanations with the exact rows in this clinic and one failure mode that would hurt the feed.
