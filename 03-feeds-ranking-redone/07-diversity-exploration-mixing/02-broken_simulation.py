...
def raw_score_order(items): return sorted(items, key=lambda item: item["score"], reverse=True)
def run(dataset):
    data=load_dataset(dataset); ranked=raw_score_order(data["content"]); final=[item["id"] for item in ranked[:5]]
    return {"inventory":[item["id"] for item in data["content"]],"eligible_ids":final,"rejected_items":[],"ranked_items":final,"final_feed":final,"author_counts":dict(Counter(item["author"] for item in ranked[:5])),"topic_counts":dict(Counter(item["topic"] for item in ranked[:5])),"debug_trace":[]}
