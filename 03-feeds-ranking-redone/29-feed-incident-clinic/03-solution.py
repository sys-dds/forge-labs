...
def run(dataset):
    data=load_dataset(dataset); bad=reproduce_incident(data); symptoms=classify_symptom(bad); causes=diagnose_root_causes(symptoms); plan=build_mitigation_plan(causes); corrected=apply_mitigations(data,plan); checks=run_regression_checks(corrected,causes)
