...
def run_reranker(rows):
    ...
    chosen=[]; authors=set()
    if 101 in by_id:
        chosen.append(101); authors.add(by_id[101]["author"])
    for r in sorted(rows,key=lambda r:(r.get("fresh",99),-r["heavy"])):
        if r["id"] == 101 or r.get("exploration"):
            continue
        if r["author"] not in authors:
            chosen.append(r["id"]); authors.add(r["author"])
...
def run(dataset):
    data=load_dataset(dataset); rows=run_retrieval_stage(data); safe,rejected=run_safety_prefilter(rows); pre=run_lightweight_pre_ranker(safe); shortlist=select_heavy_ranker_shortlist(pre); heavy=run_heavy_ranker(shortlist); final=run_reranker(heavy); timings=compute_stage_timings(len(shortlist))
