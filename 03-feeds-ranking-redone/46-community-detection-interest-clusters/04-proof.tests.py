...
def check(result):
    ...
    assert_equal(result.get("graph_candidates"), ["post_101", "post_201", "post_301"], "expected cluster overlap candidates")
    assert_true(any(row.get("entity") == "post_201" and row.get("cluster") == "movie_cluster" and row.get("strength") == 0.5 for row in result.get("community_rows", [])), "hybrid post_201 must expose movie cluster strength")
    assert_rejected(result, "post_401", "low_cluster_affinity")
    assert_rejected(result, "post_501", "unsafe_content")
    assert_graph_trace(result, "post_201", "cluster overlap")
...
def main():
    ...
    print("PASS feed clinic proof: Community detection interest clusters")
