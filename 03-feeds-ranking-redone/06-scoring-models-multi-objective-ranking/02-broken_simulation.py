...
def rank_by_like_probability(items): return sorted(items, key=lambda item: item["like"], reverse=True)
def run(dataset):
    data=load_dataset(dataset); ranked=rank_by_like_probability(data["content"]); final=[item["id"] for item in ranked]
    return {"inventory":[item["id"] for item in data["content"]],"eligible_ids":final,"rejected_items":[],"score_rows":[],"score_components":{},"ranked_items":final,"final_feed":final,"debug_trace":[]}
