...
def run(dataset_path):
    data=load_dataset(dataset_path); ranked=sorted(data["content"], key=lambda r:r.get("clicks",0), reverse=True); ids=[r["id"] for r in ranked[:4]]
    return {"inventory":inventory(data),"eligible_candidates":ranked,"rejected_items":[],"home_recommendations":ids,"up_next":ids,"final_feed":ids,"debug_trace":[]}
