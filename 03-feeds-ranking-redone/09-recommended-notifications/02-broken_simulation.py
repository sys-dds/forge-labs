...
def collect_every_notification(items): return list(items)
def run(dataset):
    data=load_dataset(dataset); sent=[item["id"] for item in collect_every_notification(data["content"])]
    return {"inventory":sent,"eligible_ids":sent,"rejected_items":[],"sent_notifications":sent,"final_feed":sent,"debug_trace":[]}
