...
def run(dataset):
    ...
    selected=[r["id"] for s,r in sorted(scored,key=lambda x:-x[0]) if s>=70 and not r.get("exploration")][:1]
    selected=preserve_secondary_interest([r for _,r in scored],selected); selected=reserve_exploration_slot([r for _,r in scored],selected)
    report=build_feedback_loop_report([104,201])
