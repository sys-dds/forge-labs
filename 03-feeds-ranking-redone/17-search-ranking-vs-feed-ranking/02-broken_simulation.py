...
def run(dataset_path):
    data=load_dataset(dataset_path); ranked=sorted(data["content"], key=lambda r:r.get("engagement",0), reverse=True); ids=[r["id"] for r in ranked[:5]]
    return {"inventory":inventory(data),"eligible_candidates":ranked,"rejected_items":[],"search_results":ids,"final_feed":ids,"debug_trace":[]}
