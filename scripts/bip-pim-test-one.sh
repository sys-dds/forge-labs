...
set -euo pipefail
clinic="${1:-}"
if [[ -z "$clinic" || ! -d "$clinic" ]]; then echo "usage: $0 04-backend-interview-patterns-redone/02-product-interaction-data-modeling-patterns/001-profile-visibility-and-profile-links" >&2; exit 2; fi
...
name="$(basename "$clinic")"; num="${name%%-*}"; schema="bip_pim_$num"
...
tmpdir="$(mktemp -d /tmp/bip-pim-test.XXXXXX)"
cleanup(){ rm -rf "$tmpdir"; }
trap cleanup EXIT
if grep -Eiq "where[[:space:][:alnum:]_\.]*_id[[:space:]]+in[[:space:]]*\(|order[[:space:]]+by[[:space:]]+case[[:space:]][[:alnum:]_\.]*_id|expected_output|expected-output|create[[:space:]]+(temporary[[:space:]]+|temp[[:space:]]+)?table[[:space:]]+expected|from[[:space:]]+expected" "$clinic/05-verification-query.sql"; then
...
run_broken
echo "PASS BIP PIM clinic proof: $clinic"
