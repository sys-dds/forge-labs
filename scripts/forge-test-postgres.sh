...
if [[ -z "$chapter_path" || ! -d "$chapter_path" ]]; then
  echo "usage: $0 02-postgres/17-advisory-locks-local-coordination" >&2
  exit 2
fi
reset_postgres_db
printf 'Testing PostgreSQL chapter %s
' "$chapter_path"
for sql_file in 01-schema.sql 02-seed.sql 03-demo.sql 04-solution.sql 05-proof.tests.sql; do
...
printf 'PASS PostgreSQL proof: %s
' "$chapter_path"
