...
for f in README.md 00-how-to-study.md 00-product-interaction-pattern-map.md 00-common-data-modeling-traps.md 00-objects-relationships-events-read-models.md 00-feed-notification-activity-patterns.md 00-recommendation-candidate-basics.md; do [[ -s "$root/$f" ]] || fail "missing $root/$f"; done
...
[[ "$(./scripts/bip-pim-list.sh | wc -l | tr -d ' ')" -eq 10 ]] || fail "all-test list must include exactly 001-010"
