...
[[ "$count" -eq 25 ]] || fail "expected 25 clinics, found $count"
if find "$root" -maxdepth 1 -type d -name '026-*' | grep -q .; then fail "unexpected clinics beyond 025"; fi
...
[[ "$(./scripts/bip-pim-list.sh | wc -l | tr -d ' ')" -eq 25 ]] || fail "all-test list must include exactly 001-025"
