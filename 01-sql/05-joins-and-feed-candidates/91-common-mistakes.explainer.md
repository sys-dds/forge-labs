...
- Mistake: inner join removes post 102 because it has zero comments.
- Why it feels reasonable: in Joins and Feed Candidates, the happy path still returns something because Ben post 101 row multiplication exists, but mistake 1 ignores the row that should constrain the answer.
- The production bug it creates: Ben post 101 row multiplication is mishandled in a way that a real endpoint would expose to a user or ranking job.
