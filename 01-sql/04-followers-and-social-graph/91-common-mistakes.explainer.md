...
- The mistake 1: reversing follower_id and followee_id.
- Why it feels reasonable 1: in Followers and Social Graph, the happy path still returns something because Ada suggestions should be Diya exists, but mistake 1 ignores the row that should constrain the answer.
- The production bug it creates 1: Ada suggestions should be Diya is mishandled in a way that a real endpoint would expose to a user or ranking job.
