...
- The mistake 1: profile table with separate id but no unique user_id.
- Why it feels reasonable 1: in Tables and Relationships, the happy path still returns something because Ada duplicate profile exists, but mistake 1 ignores the row that should constrain the answer.
- The production bug it creates 1: Ada duplicate profile is mishandled in a way that a real endpoint would expose to a user or ranking job.
