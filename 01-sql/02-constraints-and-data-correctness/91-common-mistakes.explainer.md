...
- Mistake: app-only duplicate follow check.
- Why it feels reasonable: in Constraints and Data Correctness, the happy path still returns something because Ada to Ben duplicate follow exists, but mistake 1 ignores the row that should constrain the answer.
- The production bug it creates: Ada to Ben duplicate follow is mishandled in a way that a real endpoint would expose to a user or ranking job.
