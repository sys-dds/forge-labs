...
CREATE TABLE swipes (swiper_id integer REFERENCES users(id), swiped_id integer REFERENCES users(id), action text CHECK (action IN ('LIKE','PASS')), PRIMARY KEY (swiper_id, swiped_id), CHECK (swiper_id<>swiped_id));
